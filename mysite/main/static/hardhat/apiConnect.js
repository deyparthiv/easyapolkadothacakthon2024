...
async function getTokenTransferEventsByAddress(address){
   const res = await fetch(`https://api-moonbeam.moonscan.io/api?module=account&action=tokentx&address=${address}startblock=0&endblock=latest&sort=asc&apikey=${MoonbaseKey}`)
   const data = await res.json();
   if(data.status!='1'){throw new Error(`HTTP Error! Status: ${res.status}`)}
   return data.result
}
async function getTokenTransferEventsByAddressAndContractAddress(address,contractAddress){
    const res = await fetch(`https://api-moonbeam.moonscan.io/api?module=account&action=tokentx&contractaddress=${contractAddress}&address=${address}&startblock=0&endblock=latest&sort=asc&apikey=${MoonbaseKey}`)
    const data = await res.json();
    if(data.status!='1'){throw new Error(`HTTP Error! Status: ${res.status}`)}
    return data.result
 }
